...
- `crackme_sallos_keylicense.yar`
  Detects the **"Sallos's Key License"** crackme used in `reverse-engineering/sallos-key-license/`.
  The rule leverages:
...
- `gpgcrypt_ransomware_binary.yar`
  Detects the **GPGcryptor lab ransomware binary** analyzed in `malware-analysis/gpgcryptor/`:
...
yara -r detection/yara/gpgcrypt_encrypted_file.yar /path/to/directory

# Scan a specific binary (e.g. the crackme)
yara -s detection/yara/crackme_sallos_keylicense.yar reverse-engineering/sallos-key-license/keylicense.exe
```
